...
class Board(object):
    ...
    def __repr__(self):
        ...
        string_board = str(self.__dim) # Initialize a string for a board by adding its dimension number as a string
        for i, tile in enumerate(self.__board): # Use i to count the number of tiles with tile which is every character in self.__board
            if (i % self.__dim) == 0: # If the number of tiles is a multiple of the dimension
                string_board += '\n' # Skip to a new line 
            string_board += '{:2d} '.format(tile) # Add that tile to the board string
        string_board += '\n' # Add a line at the end to match expected output
        return string_board     # YOU MUST IMPLEMENT THIS!
